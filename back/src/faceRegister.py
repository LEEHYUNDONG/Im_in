...
# resnet을 pretrained된 것을 사용한다.
resnet = InceptionResnetV1(pretrained='vggface2').eval()

dataset = datasets.ImageFolder('./face')  # 사진 폴더 경로
# folder내의 사진과 폴더명들을 각각 dict 형태로 저장
...
for i in range(0, 10):
    ...
    for j in range(0, 10):
        ...
        img_cropped = mtcnn(img, save_path="./croppedFace/" +
                            idx_to_class[i] + "/" + str(j+1) + ".jpg")
...
for i in range(0, 10):
    ...
    img_cropped = mtcnn(
        img, save_path="./croppedTestFace/" + test_file_list[i])
...
def face_match(img_path, data_path):  # img_path= 사진의 위치, data_path= 모델의 위치
    ...
    # detach는 required grad를 false로 넘겨준다.
